    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect('{}#client_answer_{}'.format(
                resolve_url('kdp:client_detail', client_id=client.id), client_answer.id))
    else:
...
def client_answer_modify(request, client_answer_id):
    ...
    if request.user != client_answer.author:
        ...
        return redirect('kdp:client_detail', client_id=client_answer.client.id)
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            client_answer.save()
            return redirect('{}#client_answer_{}'.format(
                resolve_url('kdp:client_detail', client_id=client_answer.client.id), client_answer.id))
    else:
        form = ClientAnswerForm(instance=client_answer)
    context = {'client_answer': client_answer, 'form': form}
    return render(request, 'kdp/clientanswer_form.html', context)
